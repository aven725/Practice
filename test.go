...
type OpenFlowLayer struct {
	...
	oft_version uint8
	ofp_type    uint8 //ofp4.Enum_ofp_type
	ofp_lenght  uint16
	ofp_xid     uint32
	ofp_payload []byte
}
...
func (l OpenFlowLayer) LayerVersion() byte {
	return byte(l.oft_version)
}
...
func decodeOpenFlowLayer(data []byte, p gopacket.PacketBuilder) error {
	...
	p.AddLayer(&OpenFlowLayer{uint8(data[0]), uint8(data[1]), binary.BigEndian.Uint16(data[2:3]), binary.BigEndian.Uint32(data[4:7]), data[8:10]})
...
func main() {
	...
	//	rawBytes := []byte{0x04, 0x0a, 0x00, 0x54, 0x00, 0x00, 0x00, 0x00}
